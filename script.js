...
async function run() {
  ...
  // category 변수 처리 (Male -> 1, Female -> 0)
  dataPoints.map((v) => {
    if (v.x[0] === "Male") {
      v.x[0] = 1;
  ...
  // train/test dataset 분리 (75% train, 25% test)
  ...
  const normedTestTensor = MinMaxScaling(
    X_test,
    normParams.min,
    normParams.max
  );
...
async function train() {
  // layers API를 이용하여 sequential model 구축
  ...
  document.getElementById("model-status").innerHTML =
    "train comeplete, start prediction !!";

  // confusion matrix 생성 및 시각화
  const predictions = model.predict(X_test);
  const predClasses = tf.tidy(() =>
    tf.floor(predictions.add(0.5)).transpose().squeeze()
  );
  const trueClasses = tf.tidy(() =>
    tf.floor(y_test.add(0.5)).transpose().squeeze()
  );
...
async function predict() {
  ...
  if (isNaN(predInputOne) || isNaN(predInputTwo) || isNaN(predInptThree)) {
    ...
    const feature = [predInputOne, predInputTwo, predInptThree];
    // tensor 변환
    const tempTensor = tf.tensor2d(feature, [1, 3]);
